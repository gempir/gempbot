...
func (ec *EmoteChief) VerifySeventvRedemption(reward store.ChannelPointReward, redemption helix.EventSubChannelPointsCustomRewardRedemptionEvent) bool {
	...
	if err == nil {
		...
		if err != nil {
			...
			ec.helixClient.SendChatMessage(redemption.BroadcasterUserID, fmt.Sprintf("⚠️ Failed to add 7TV emote from @%s error: %s", redemption.UserName, err.Error()))
			return false
		}
...
func (ec *EmoteChief) HandleSeventvRedemption(reward store.ChannelPointReward, redemption helix.EventSubChannelPointsCustomRewardRedemptionEvent, updateStatus bool) {
	...
	if err == nil {
		...
		if settingErr != nil {
			...
			ec.helixClient.SendChatMessage(redemption.BroadcasterUserID, fmt.Sprintf("⚠️ Failed to add 7TV emote from @%s %s", redemption.UserName, settingErr.Error()))
		} else if addedEmote.Code != "" && removedEmote.Code != "" {
			success = true
			ec.helixClient.SendChatMessage(redemption.BroadcasterUserID, fmt.Sprintf("✅ Added new 7TV emote %s redeemed by @%s removed %s", addedEmote.Code, redemption.UserName, removedEmote.Code))
		} else if addedEmote.Code != "" {
			success = true
			ec.helixClient.SendChatMessage(redemption.BroadcasterUserID, fmt.Sprintf("✅ Added new 7TV emote %s redeemed by @%s", addedEmote.Code, redemption.UserName))
		} else {
			success = true
			ec.helixClient.SendChatMessage(redemption.BroadcasterUserID, fmt.Sprintf("✅ Added new 7TV emote [unknown] redeemed by @%s", redemption.UserName))
		}
	} else {
		ec.helixClient.SendChatMessage(redemption.BroadcasterUserID, fmt.Sprintf("⚠️ Failed to add 7TV emote from @%s %s", redemption.UserName, err.Error()))
	}
