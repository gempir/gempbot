...
export function useSubscribtions(): [
  ...
  const [subscriptionStatus, setSubscriptionStatus] =
    useState<SubscriptionStatus>({ predictions: false });
  ...
  const subscribe = () => {
    ...
    executeSubscriptions(Method.PUT)
      .then(() => setSubscriptionStatus({ predictions: true }))
  ...
  const remove = () => {
    ...
    executeSubscriptions(Method.DELETE)
      .then(() => setSubscriptionStatus({ predictions: false }))
      .then(() => setLoading(false))
      .catch((err) => {
        console.error(err);
        setLoading(false);
  ...
  const status = () => {
    ...
    executeSubscriptions(Method.GET)
