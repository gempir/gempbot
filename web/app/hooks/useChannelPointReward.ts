...
export function useChannelPointReward(
  _userID: string,
  type: RewardTypes,
  defaultReward: ChannelPointReward,
): [
  ChannelPointReward,
  (reward: ChannelPointReward) => void,
  () => void,
  string | null,
  boolean,
] {
      ...
      .then((response: RawSevenTvChannelPointReward) => ({
        ...
        AdditionalOptionsParsed:
          response.AdditionalOptions !== ""
            ? JSON.parse(response.AdditionalOptions)
            : defaultReward.AdditionalOptionsParsed,
      }))
      .then(setReward)
      .catch((reason) => {
