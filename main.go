package main

import (
	"net/http"

	"github.com/gempir/gempbot/internal/auth"
	"github.com/gempir/gempbot/internal/bot"
	"github.com/gempir/gempbot/internal/channelpoint"
	"github.com/gempir/gempbot/internal/config"
	"github.com/gempir/gempbot/internal/emotechief"
	"github.com/gempir/gempbot/internal/emoteservice"
	"github.com/gempir/gempbot/internal/eventsub"
	"github.com/gempir/gempbot/internal/helixclient"
	"github.com/gempir/gempbot/internal/log"
	"github.com/gempir/gempbot/internal/server"
	"github.com/gempir/gempbot/internal/store"
	"github.com/gempir/gempbot/internal/user"
	"github.com/newrelic/go-agent/v3/newrelic"
	"github.com/rs/cors"
)

func main() {
	cfg := config.FromEnv()
	db := store.NewDatabase(cfg)

	helixClient := helixclient.NewClient(cfg, db)
	go helixClient.StartRefreshTokenRoutine()

	userAdmin := user.NewUserAdmin(cfg, db, helixClient, nil)
	authClient := auth.NewAuth(cfg, db, helixClient)

	bot := bot.NewBot(cfg, db, helixClient)
	go bot.Connect()

	seventvClient := emoteservice.NewSevenTvClient(cfg.SevenTvToken)

	emoteChief := emotechief.NewEmoteChief(cfg, db, helixClient, bot.ChatClient, seventvClient)
	eventsubManager := eventsub.NewEventsubManager(cfg, helixClient, db, emoteChief, bot.ChatClient)
	eventsubSubscriptionManager := eventsub.NewSubscriptionManager(cfg, db, helixClient)
	channelPointManager := channelpoint.NewChannelPointManager(cfg, helixClient, db)

	apiHandlers := server.NewApi(cfg, db, helixClient, userAdmin, authClient, bot, emoteChief, eventsubManager, eventsubSubscriptionManager, channelPointManager, seventvClient)

	nrapp, err := newrelic.NewApplication(
		newrelic.ConfigAppName("gempbot"),
		newrelic.ConfigLicense(cfg.NewrelicLicense),
		newrelic.ConfigDistributedTracerEnabled(true),
	)
	if err != nil {
		log.Error(err)
	}

	mux := http.NewServeMux()

	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		_, _ = w.Write([]byte("<img src=\"data:image/webp;base64,\" />"))
		w.WriteHeader(http.StatusOK)
	}))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/blocks", apiHandlers.BlocksHandler))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/botconfig", apiHandlers.BotConfigHandler))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/callback", apiHandlers.CallbackHandler))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/emotehistory", apiHandlers.EmoteHistoryHandler))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/eventsub", apiHandlers.EventSubHandler))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/reward", apiHandlers.RewardHandler))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/subscriptions", apiHandlers.SubscriptionsHandler))
	mux.HandleFunc(newrelic.WrapHandleFunc(nrapp, "/api/userconfig", apiHandlers.UserConfigHandler))

	handler := cors.New(cors.Options{
		AllowedOrigins:   []string{cfg.WebBaseUrl},
		AllowedMethods:   []string{"GET", "POST", "PUT", "DELETE", "PATCH"},
		AllowedHeaders:   []string{"*"},
		AllowCredentials: true,
	}).Handler(mux)
	err = http.ListenAndServe(":3010", handler)
	if err != nil {
		log.Fatal(err)
	}
}
